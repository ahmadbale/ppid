    ...
    <div class="card card-outline card-primary">
        <div class="card-header">
            <div class="row align-items-center">
                <div class="col-md-6">
                    <h3 class="card-title">{{ $pageTitle ?? 'Data Master' }}</h3>
                </div>
                <div class="col-md-6 text-right">
                    <div class="card-tools">
                        @if(
                            Auth::user()->level->hak_akses_kode === 'SAR' ||
                            SetHakAksesModel::cekHakAkses(Auth::user()->user_id, $menuUrl, 'create')
                        )
                            <button onclick="modalAction('{{ url($menuUrl . '/addData') }}')" class="btn btn-sm btn-success">
        ...
        <div class="card-body">
            ...
            <div class="table-responsive" id="table-container">
                @include('sisfo::Template.Master.data')
            </div>
        </div>
    </div>
    ...
    <script>
        $(document).ready(function () {
            ...
            $(document).on('click', '.pagination a', function (e) {
                ...
                loadMasterData(page, search);
            });

            // Fungsi untuk memuat data Master
            function loadMasterData(page, search) {
                $.ajax({
            ...
            // Fungsi modalAction yang seragam dengan custom menu
            ...
            function reloadTable() {
                ...
                var search = $('#searchForm input[name="search"]').val();
                loadMasterData(currentPage, search);
            }

            // Expose fungsi-fungsi ke lingkup global
