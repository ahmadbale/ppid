  ...
  <div class="card card-outline card-primary">
          ...
          <div class="col-md-6 text-right">
            <div class="card-tools">
              ...
              @if(
                ...
                SetHakAksesModel::cekHakAkses(Auth::user()->user_id, $penyelesaianSengketaUrl, 'create')
                )
                <button onclick="modalAction('{{ url($penyelesaianSengketaUrl . '/addData') }}')" class="btn btn-sm btn-success">
      ...
      <div class="card-body">
          ...
          <div class="col-md-6">
            <form id="searchForm" class="d-flex">
              <input type="text" name="search" class="form-control" 
                     placeholder="Cari Kode atau nama penyelesaian sengketa" 
                     value="{{ $search ?? '' }}">
        ...
        <div class="table-responsive" id="table-container">
          @include('sisfo::AdminWeb.InformasiPublik.PenyelesaianSengketa.data')
        </div>
      </div>
  </div>
...
<script>
  ...
  var penyelesaianSengketaUrl = '{{ $penyelesaianSengketaUrl }}';

  // Handle search form submission
  $('#searchForm').on('submit', function (e) {
    e.preventDefault();
    var search = $(this).find('input[name="search"]').val();
    loadPenyelesaianSengketa(1, search);
  });
  ...
  $(document).on('click', '.pagination a', function (e) {
    ...
    loadPenyelesaianSengketa(page, search);
  });

  // Fungsi untuk memuat data kategoriTahunLHKPN
  function loadPenyelesaianSengketa(page, search) {
    $.ajax({
    url: '{{ url('') }}/' + penyelesaianSengketaUrl + '/getData',
    type: 'GET',
    data: {
      page: page,
      search: search
  ...
  function reloadTable() {
    ...
    var search = $('#searchForm input[name="search"]').val();
    loadPenyelesaianSengketa(currentPage, search);
  }
