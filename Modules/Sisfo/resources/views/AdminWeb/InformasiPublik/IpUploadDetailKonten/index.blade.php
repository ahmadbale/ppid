  ...
  <div class="card card-outline card-primary">
          ...
          <div class="col-md-6 text-right">
            <div class="card-tools">
              ...
              @if(
                ...
                SetHakAksesModel::cekHakAkses(Auth::user()->user_id, $IpUploadKontenUrl, 'create')
                )
                <button onclick="modalAction('{{ url($IpUploadKontenUrl . '/addData') }}')" class="btn btn-sm btn-success">
      ...
      <div class="card-body">
        ...
        <div class="table-responsive" id="table-container">
          @include('sisfo::AdminWeb.InformasiPublik.IpUploadDetailKonten.data')
        </div>
      </div>
  </div>
...
<script>
  ...
  var IpdinamisTabelUrl = '{{ $IpUploadKontenUrl }}';

  // Handle search form submission
  $('#searchForm').on('submit', function (e) {
    e.preventDefault();
    var search = $(this).find('input[name="search"]').val();
    loadIPUploadKontenData(1, search);
  });
  ...
  $(document).on('click', '.pagination a', function (e) {
    ...
    loadIPUploadKontenData(page, search);
  });

  // Fungsi untuk memuat data KategoriFooter
  function loadIPUploadKontenData(page, search) {
    $.ajax({
    url: '{{ url('') }}/' + IpdinamisTabelUrl + '/getData',
    type: 'GET',
    data: {
      page: page,
      search: search
  ...
  function reloadTable() {
    ...
    var search = $('#searchForm input[name="search"]').val();
    loadIPUploadKontenData(currentPage, search);
  }
