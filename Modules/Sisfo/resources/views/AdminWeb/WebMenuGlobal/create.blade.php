...
{{-- ========================================================
     CUSTOM SEARCHABLE SELECT — CSS (inject sekali per halaman)
     Gunakan @once atau letakkan di layout/section styles jika perlu
     ======================================================== --}}
<style>
...
.ss-option-item.hidden   { display: none; }
...
<div class="modal-body">
    <form id="formCreateWebMenuGlobal" action="{{ url($webMenuGlobalPath . '/createData') }}" method="POST">
        ...
        <div class="form-group" id="menu_url_group">
            ...
            <input type="hidden" id="fk_web_menu_url" name="web_menu_global[fk_web_menu_url]" value="">

            <div class="ss-wrapper" id="ssWrapCreate">
                {{-- Trigger / display --}}
                <div class="ss-trigger" id="ssTriggerCreate" tabindex="0" role="combobox"
                     aria-haspopup="listbox" aria-expanded="false">
                    <div class="ss-display placeholder" id="ssDisplayCreate">Pilih Menu URL</div>
                    <div class="ss-actions">
                        <button type="button" class="ss-clear-btn" id="ssClearCreate" title="Hapus pilihan">
                ...
                {{-- Dropdown panel --}}
                <div class="ss-dropdown" id="ssDropdownCreate" role="listbox">
                    <div class="ss-search-wrap">
                        <input type="text" class="ss-search-input" id="ssSearchCreate"
                               placeholder="Cari nama menu, URL, atau deskripsi..." autocomplete="off">
                    </div>
                    <div class="ss-stats" id="ssStatsCreate">
                        Menampilkan semua — terbaru di atas
                    </div>
                    <div class="ss-options-list" id="ssOptionListCreate">
                        {{-- Diisi oleh JS dari data JSON di bawah --}}
                    </div>
                </div>
            </div>
...
$(document).ready(function () {
    ...
    const menuUrlData = [
        @foreach(array_reverse($menuUrls->toArray()) as $index => $url)
        {
            ...
            isNew: {{ $index < 5 ? 'true' : 'false' }}   {{-- 5 item pertama (terbaru) dapat badge --}}
        },
        @endforeach
    ];

    /* ══════════════════════════════════════════════════════════════
       SEARCHABLE SELECT — factory function (dapat dipakai ulang)
       ══════════════════════════════════════════════════════════════ */
    function initSearchableSelect(cfg) {
        /*
         * cfg = {
         *   data        : array of {value, app, name, desc, isNew}
         *   hiddenInput : jQuery selector string  → input[type=hidden]
         *   trigger     : jQuery selector string  → .ss-trigger
         *   display     : jQuery selector string  → .ss-display
         *   clearBtn    : jQuery selector string  → .ss-clear-btn
         *   dropdown    : jQuery selector string  → .ss-dropdown
         *   searchInput : jQuery selector string  → .ss-search-input
         *   stats       : jQuery selector string  → .ss-stats
         *   optionList  : jQuery selector string  → .ss-options-list
         *   errorDiv    : jQuery selector string  → .invalid-feedback
         *   selectedValue: string (pre-selected value, optional)
         * }
         */
        ...
        /* ── Render semua option berdasarkan query ─ */
        ...
        // Click outside
        $(document).on('click.ss_' + cfg.trigger.replace(/[^a-z0-9]/gi,''), function(e) {
        ...
        // Init display jika ada nilai awal
        if (currentValue) updateDisplay(currentValue);
        else renderOptions('');
    }

    /* ══════════════════════════════════════════════════════════════
       INISIALISASI untuk form CREATE
       ══════════════════════════════════════════════════════════════ */
    ...
    $('#btnSubmitForm').on('click', function() {
        ...
        if ((kategori === 'Sub Menu' || kategori === 'Menu Biasa') && !urlVal) {
            $('#ssTriggerCreate').addClass('is-invalid');
