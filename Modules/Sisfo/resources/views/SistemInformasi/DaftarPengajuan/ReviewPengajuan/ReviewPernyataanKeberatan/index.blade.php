...
@section('content')
    <div class="card card-primary card-outline">
        <div class="card-header">
            <h3 class="card-title">Daftar Review Pengajuan Pernyataan Keberatan</h3>
        </div>
        <div class="card-body">
            <div class="mb-3">
            ...
            @if(count($pernyataanKeberatan) > 0)
                <div class="table-responsive">
                    @include('sisfo::SistemInformasi.DaftarPengajuan.ReviewPengajuan.ReviewPernyataanKeberatan.data')
                </div>
            @else
                <div class="alert alert-info">
                    <i class="fas fa-info-circle mr-2"></i> Tidak ada data pernyataan keberatan yang perlu diReview.
                </div>
            @endif
        </div>
    </div>
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="detailModalLabel">Detail Pernyataan Keberatan</h5>
    ...
    <script>
        $(function () {
            ...
            window.showApproveModal = function (id) {
                // Reset modal content
                $('#detailModalLabel').text('Persetujuan Review Pernyataan Keberatan');
                ...
                $.ajax({
                    url: '{{ url("$daftarReviewPengajuanUrl/pernyataan-keberatan/approve-modal") }}/' + id,
                    type: 'GET',
                    success: function (response) {
                        ...
                        $('#btnConfirmApprove').on('click', function () {
                            ...
                            const pernyataanKeberatanId = $(this).data('id');
                            ...
                            Swal.fire({
                                title: 'Konfirmasi',
                                text: 'Anda yakin sudah mereview data dan akan menyetujui review pernyataan keberatan ini?',
                                icon: 'question',
                                ...
                                if (result.isConfirmed) {
                                    ...
                                    $.ajax({
                                        url: '{{ url("$daftarReviewPengajuanUrl/pernyataan-keberatan/setujuiPermohonan") }}/' + pernyataanKeberatanId,
                                        type: 'POST',
                                        data: formData,
                                        processData: false,
                                        contentType: false,
            ...
            window.showDeclineModal = function (id) {
                // Reset modal content
                $('#detailModalLabel').text('Penolakan Review Pernyataan Keberatan');
                $('#modalContent').html('<div class="text-center py-5"><i class="fas fa-spinner fa-spin fa-3x"></i><p class="mt-2">Memuat data...</p></div>');
                ...
                // Ambil konten modal via AJAX
                $.ajax({
                    url: '{{ url("$daftarReviewPengajuanUrl/pernyataan-keberatan/decline-modal") }}/' + id,
                    type: 'GET',
                    success: function (response) {
                        $('#modalContent').html(response);
                        ...
                        $('#btnConfirmDecline').on('click', function () {
                            ...
                            Swal.fire({
                                title: 'Konfirmasi',
                                text: 'Jika anda yakin telah melakukan review dan akan menolak pernyataan keberatan ini dengan alasan yang telah diisi?',
                                icon: 'question',
                                showCancelButton: true,
                                ...
                                if (result.isConfirmed) {
                                    ...
                                    // Kirim permintaan AJAX
                                    $.ajax({
                                        url: '{{ url("$daftarReviewPengajuanUrl/pernyataan-keberatan/tolakPermohonan") }}/' + pernyataanKeberatanId,
                                        type: 'POST',
                                        data: {
                                            _token: '{{ csrf_token() }}',
            ...
            // Fungsi untuk tandai dibaca review
            window.tandaiDibacaReview = function (id, status, sudahDibaca) {
                // Validasi awal: review harus sudah disetujui atau ditolak
                ...
                Swal.fire({
                    title: 'Konfirmasi',
                    text: 'Apakah anda yakin ingin menandai review pernyataan keberatan ini sebagai telah dibaca?',
                    icon: 'question',
                    showCancelButton: true,
                    ...
                    if (result.isConfirmed) {
                        ...
                        // Kirim permintaan AJAX
                        $.ajax({
                            url: '{{ url("$daftarReviewPengajuanUrl/pernyataan-keberatan/tandaiDibaca") }}/' + id,
                            type: 'POST',
                            data: {
                                _token: '{{ csrf_token() }}'
                            },
            ...
            window.hapusReview = function (id, sudahDibaca) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        // Kirim permintaan AJAX
                        $.ajax({
                            url: '{{ url("$daftarReviewPengajuanUrl/pernyataan-keberatan/hapusPermohonan") }}/' + id,
                            type: 'DELETE',
                            data: {
                                _token: '{{ csrf_token() }}'
                            },
