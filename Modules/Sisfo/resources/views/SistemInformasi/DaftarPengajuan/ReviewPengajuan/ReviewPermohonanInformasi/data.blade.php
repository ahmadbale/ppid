...
<div class="card shadow-sm mb-4">
    ...
    <div class="card-body">
        ...
        <div class="tab-content" id="myTabContent-{{ $PI->permohonan_informasi_id }}">
            ...
            <!-- Tab Data Pemohon - TIDAK BERUBAH -->
            ...
            <div class="tab-pane fade" id="status-{{ $PI->permohonan_informasi_id }}" role="tabpanel" 
                            ...
                            <div class="card-body">
                                <table class="table table-borderless table-sm">
                                    ...
                                    <tr>
                                        ...
                                        <td>: {{ $PI->pi_review_tanggal_dibaca ? \Carbon\Carbon::parse($PI->pi_review_tanggal_dibaca)->format('d-m-Y H:i:s') : '-' }}</td>
                                    </tr>
                                    <tr>
                                        <td class="font-weight-bold">Tanggal Jawaban</td>
                                        <td>: {{ $PI->pi_tanggal_dijawab ? \Carbon\Carbon::parse($PI->pi_tanggal_dijawab)->format('d-m-Y H:i:s') : '-' }}</td>
                                    </tr>
                                </table>
                            </div>
                        </div>
                ...
                @if($PI->pi_jawaban && in_array($PI->pi_status, ['Disetujui', 'Ditolak']))
                <div class="row">
                    <div class="col-md-12">
                        <div class="card bg-light mb-3">
                            <div class="card-header {{ $PI->pi_status == 'Disetujui' ? 'bg-success' : 'bg-danger' }} text-white">
                                <h6 class="m-0">
                                    <i class="fas {{ $PI->pi_status == 'Disetujui' ? 'fa-check-circle' : 'fa-times-circle' }} mr-1"></i> 
                                    {{ $PI->pi_status == 'Disetujui' ? 'Jawaban Permohonan' : 'Alasan Penolakan' }}
                                </h6>
                            </div>
                            <div class="card-body">
                                @if($PI->pi_status == 'Disetujui' && preg_match('/\.(pdf|doc|docx|jpg|jpeg|png|gif)$/i', $PI->pi_jawaban))
                                    {{-- Jika jawaban berupa file --}}
                                    <div class="text-center">
                                        <a href="{{ asset('storage/' . $PI->pi_jawaban) }}" target="_blank" 
        ...
        <!-- Tombol Aksi - INI YANG DIPERBAIKI -->
            ...
            <div class="row">
                <div class="col-12">
                    <div class="d-flex flex-wrap">
                        ...
                        {{-- PERBAIKAN: Update kondisi untuk tandai dibaca dan hapus --}}
                        <button class="btn btn-info btn-sm mr-2 mb-2" 
                            onclick="tandaiDibacaReview({{ $PI->permohonan_informasi_id }}, '{{ $PI->pi_status }}', {{ $PI->pi_review_sudah_dibaca ? 'true' : 'false' }})" 
                            data-status="{{ $PI->pi_status }}" 
                            data-dibaca="{{ $PI->pi_review_sudah_dibaca ? 'true' : 'false' }}">
                            <i class="fas fa-book-reader"></i> Tandai Review Dibaca
                        </button>
                        
                        <button class="btn btn-warning btn-sm mb-2" 
                            onclick="hapusReview({{ $PI->permohonan_informasi_id }}, {{ $PI->pi_review_sudah_dibaca ? 'true' : 'false' }})" 
                            data-dibaca="{{ $PI->pi_review_sudah_dibaca ? 'true' : 'false' }}">
