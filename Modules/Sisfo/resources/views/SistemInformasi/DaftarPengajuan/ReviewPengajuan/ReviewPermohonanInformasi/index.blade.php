...
@section('content')
    <div class="card card-primary card-outline">
        <div class="card-header">
            <h3 class="card-title">Daftar Review Pengajuan Permohonan Informasi</h3>
        </div>
        <div class="card-body">
            <div class="mb-3">
            ...
            @if(count($permohonanInformasi) > 0)
                <div class="table-responsive">
                    @include('sisfo::SistemInformasi.DaftarPengajuan.ReviewPengajuan.ReviewPermohonanInformasi.data')
                </div>
            @else
                <div class="alert alert-info">
                    <i class="fas fa-info-circle mr-2"></i> Tidak ada data permohonan informasi yang perlu diReview.
                </div>
            @endif
        </div>
    </div>
    ...
    <script>
        $(function () {
            ...
            window.showApproveModal = function (id) {
                ...
                $.ajax({
                    url: '{{ url("$daftarReviewPengajuanUrl/permohonan-informasi/approve-modal") }}/' + id,
                    type: 'GET',
                    success: function (response) {
                        ...
                        $('#btnConfirmApprove').on('click', function () {
                            ...
                            Swal.fire({
                                title: 'Konfirmasi',
                                text: 'Anda yakin sudah mereview data dan akan menyetujui permohonan ini?',
                                icon: 'question',
                                ...
                                if (result.isConfirmed) {
                                    ...
                                    $.ajax({
                                        url: '{{ url("$daftarReviewPengajuanUrl/permohonan-informasi/setujuiPermohonan") }}/' + permohonanId,
                                        type: 'POST',
                                        data: formData,
                                        processData: false,
                                        contentType: false,
            ...
            window.showDeclineModal = function (id) {
                ...
                // Ambil konten modal via AJAX
                $.ajax({
                    url: '{{ url("$daftarReviewPengajuanUrl/permohonan-informasi/decline-modal") }}/' + id,
                    type: 'GET',
                    success: function (response) {
                        $('#modalContent').html(response);
                        ...
                        $('#btnConfirmDecline').on('click', function () {
                            ...
                            Swal.fire({
                                ...
                                if (result.isConfirmed) {
                                    ...
                                    // Kirim permintaan AJAX
                                    $.ajax({
                                        url: '{{ url("$daftarReviewPengajuanUrl/permohonan-informasi/tolakPermohonan") }}/' + permohonanId,
                                        type: 'POST',
                                        data: {
                                            _token: '{{ csrf_token() }}',
            ...
            window.tandaiDibacaReview = function (id, status, sudahDibaca) {
                ...
                Swal.fire({
                    title: 'Konfirmasi',
                    text: 'Apakah anda yakin ingin menandai review permohonan informasi ini sebagai telah dibaca?',
                    icon: 'question',
                    showCancelButton: true,
                    ...
                    if (result.isConfirmed) {
                        ...
                        // Kirim permintaan AJAX
                        $.ajax({
                            url: '{{ url("$daftarReviewPengajuanUrl/permohonan-informasi/tandaiDibaca") }}/' + id,
                            type: 'POST',
                            data: {
                                _token: '{{ csrf_token() }}'
                            },
            ...
            window.hapusReview = function (id, sudahDibaca) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        // Kirim permintaan AJAX
                        $.ajax({
                            url: '{{ url("$daftarReviewPengajuanUrl/permohonan-informasi/hapusPermohonan") }}/' + id,
                            type: 'DELETE',
                            data: {
                                _token: '{{ csrf_token() }}'
                            },
