        ...
        <div class="d-flex justify-content-between align-items-center">
            ...
            <div class="d-flex align-items-center">
                ...
                <div class="text-muted">
                    <i class="far fa-clock mr-1"></i> {{ date('d M Y H:i', strtotime($WBS->created_at)) }}
                </div>
            </div>
        </div>
    </div>
                            ...
                            <div class="card-body">
                                <table class="table table-borderless table-sm">
                                    <tr>
                                        ...
                                        <td>: {{ date('d M Y H:i:s', strtotime($WBS->wbs_waktu_kejadian)) }}</td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr>
                                        ...
                                        <td>: 
                                            @if($WBS->wbs_upload_nik_pengguna)
                                                <a href="{{ asset('storage/' . $WBS->wbs_upload_nik_pengguna) }}" target="_blank" class="btn btn-sm btn-info">
                                                    <i class="fas fa-eye mr-1"></i> Lihat Identitas
                                                </a>
                                            @else
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-12">
                                ...
                                @if($WBS->wbs_bukti_pendukung)
                                    <a href="{{ asset('storage/' . $WBS->wbs_bukti_pendukung) }}" target="_blank" class="btn btn-info">
                            ...
                            <div class="card-body">
                                <table class="table table-borderless table-sm">
                                    <tr>
                                        ...
                                        <td>: {{ $WBS->created_at ? date('d-m-Y H:i:s', strtotime($WBS->created_at)) : '-' }}</td>
                                    </tr>
                                    <tr>
                                        <td class="font-weight-bold">Tanggal Dibaca</td>
                                        <td>: {{ $WBS->wbs_tanggal_dibaca ? date('d-m-Y H:i:s', strtotime($WBS->wbs_tanggal_dibaca)) : '-' }}</td>
                                    </tr>
                                </table>
                            </div>
                        </div>
            ...
            <div class="row">
                <div class="col-12">
                    <div class="d-flex flex-wrap">
                        ...
                        <button class="btn btn-info btn-sm mr-2 mb-2" onclick="tandaiDibaca({{ $WBS->wbs_id }}, '{{ $WBS->wbs_status }}', {{ $WBS->wbs_sudah_dibaca ? 'true' : 'false' }})" 
                            data-status="{{ $WBS->wbs_status }}" data-dibaca="{{ $WBS->wbs_sudah_dibaca ? 'true' : 'false' }}">
                            <i class="fas fa-book-reader"></i> Tandai Dibaca
                        </button>
                        
                        <button class="btn btn-warning btn-sm mb-2" onclick="hapusPermohonan({{ $WBS->wbs_id }}, {{ $WBS->wbs_sudah_dibaca ? 'true' : 'false' }})" 
                            data-dibaca="{{ $WBS->wbs_sudah_dibaca ? 'true' : 'false' }}">
