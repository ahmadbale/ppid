{{-- filepath: c:\laragon\www\PPID-polinema\Modules\Sisfo\resources\views\SistemInformasi\DaftarPengajuan\VerifPengajuan\VerifPermohonanPerawatan\index.blade.php --}}
@extends('sisfo::layouts.template')

@section('content')
<div class="card card-primary card-outline">
    <div class="card-header">
        <h3 class="card-title">Daftar Verifikasi Pengajuan Permohonan Perawatan Sarana Prasarana</h3>
    </div>
    <div class="card-body">
        <div class="mb-3">
        ...
        @if(count($permohonanPerawatan) > 0)
            <div class="table-responsive">
                @include('sisfo::SistemInformasi.DaftarPengajuan.VerifPengajuan.VerifPermohonanPerawatan.data')
            </div>
        @else
            <div class="alert alert-info">
                <i class="fas fa-info-circle mr-2"></i> Tidak ada data Permohonan Perawatan yang perlu diverifikasi.
            </div>
        @endif
    </div>
</div>
...
<script>
    $(function() {
        ...
        window.showApproveModal = function(id) {
            ...
            $.ajax({
                url: '{{ url("$daftarPengajuanUrl/permohonan-perawatan/approve-modal") }}/' + id,
                type: 'GET',
                    ...
                    $('#btnConfirmApprove').on('click', function() {
                        ...
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                ...
                                $.ajax({
                                    url: '{{ url("$daftarPengajuanUrl/permohonan-perawatan/setujuiPermohonan") }}/' + permohonanId,
                                    type: 'POST',
                                    data: {
                                        _token: '{{ csrf_token() }}'
                                    },
        ...
        window.showDeclineModal = function(id) {
            ...
            // Ambil konten modal via AJAX
            $.ajax({
                url: '{{ url("$daftarPengajuanUrl/permohonan-perawatan/decline-modal") }}/' + id,
                type: 'GET',
                success: function(response) {
                    $('#modalContent').html(response);
                    ...
                    $('#btnConfirmDecline').on('click', function() {
                        ...
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                ...
                                // Kirim permintaan AJAX
                                $.ajax({
                                    url: '{{ url("$daftarPengajuanUrl/permohonan-perawatan/tolakPermohonan") }}/' + permohonanId,
                                    type: 'POST',
                                    data: {
                                        _token: '{{ csrf_token() }}',
        ...
        window.tandaiDibaca = function(id, status, sudahDibaca) {
            ...
            Swal.fire({
                title: 'Konfirmasi',
                text: 'Apakah anda yakin ingin menandai pengajuan Permohonan Perawatan ini sebagai telah dibaca?',
                icon: 'question',
                showCancelButton: true,
                ...
                if (result.isConfirmed) {
                    ...
                    // Kirim permintaan AJAX
                    $.ajax({
                        url: '{{ url("$daftarPengajuanUrl/permohonan-perawatan/tandaiDibaca") }}/' + id,
                        type: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}'
                        },
        ...
        window.hapusPermohonan = function(id, sudahDibaca) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    // Kirim permintaan AJAX
                    $.ajax({
                        url: '{{ url("$daftarPengajuanUrl/permohonan-perawatan/hapusPermohonan") }}/' + id,
                        type: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}'
                        },
