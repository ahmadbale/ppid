...
class GetIPInformasiSetiapSaatController extends Controller
{
    use TraitsController;

    public $breadcrumb = 'Informasi Publik yang Wajib Diumumkan Setiap Saat';
    ...
    public function index(Request $request)
    {
        ...
        $breadcrumb = (object)[
            'title' => 'Informasi Publik yang Wajib Diumumkan Setiap Saat',
            'list' => ['Home', 'Informasi Publik', 'Informasi Setiap Saat']
        ];
        ...
        $activeMenu = 'Informasi Publik Setiap Saat';

        try {
            // Ambil data kategori Informasi Setiap Saat
            $kategoriInformasiSetiapSaat = IpDinamisTabelModel::where('ip_nama_submenu', 'Informasi Setiap Saat')
                ->where('isDeleted', 0)
                ->first();

            if (!$kategoriInformasiSetiapSaat) {
                return view('sisfo::AdminWeb.InformasiPublik.GetIPDinamisTabel.SetiapSaat.index', [
            ...
            // Ambil semua data menu utama dengan hierarki lengkap untuk kategori Informasi Setiap Saat
            $menuUtamaList = $this->getMenusByKategori($kategoriInformasiSetiapSaat->ip_dinamis_tabel_id, $search);
            ...
            $lastUpdate = $this->getLastUpdateDate($kategoriInformasiSetiapSaat->ip_dinamis_tabel_id);

            return view('sisfo::AdminWeb.InformasiPublik.GetIPDinamisTabel.SetiapSaat.index', [
                'breadcrumb' => $breadcrumb,
                'page' => $page,
                'activeMenu' => $activeMenu,
                'kategori' => $kategoriInformasiSetiapSaat,
    ...
    public function getData(Request $request)
    {
        ...
        try {
            ...
            if (!$kategoriInformasiSetiapSaat) {
                if ($request->ajax()) {
                    return view('sisfo::AdminWeb.InformasiPublik.GetIPDinamisTabel.SetiapSaat.data', [
                        'kategori' => null,
                        'menuUtamaList' => collect(),
                        'totalDokumen' => 0,
                        'totalData' => 0,
                ...
                return redirect()->back()->with('error', 'Kategori Informasi Setiap Saat tidak ditemukan');
            }
    ...
    public function detailData($id)
    {
        try {
            ...
            if ($action === 'download') {
                return response()->download(storage_path('app/public/' . $document), $filename . '.pdf');
            }

            $documentUrl = asset('storage/' . $document);
            
            return view('sisfo::AdminWeb.InformasiPublik.GetIPDinamisTabel.SetiapSaat.detail', [
