...
class PengaduanMasyarakatModel extends Model
{
    ...
    public function validasiDanTolakPermohonan($alasanPenolakan)
    {
        ...
        // TIDAK membuat notifikasi MPU untuk pengaduan yang ditolak
    ...
    {
        // Hanya menghitung review untuk Pengaduan Masyarakat
        return self::where('pm_status', 'Verifikasi')
            ->where('isDeleted', 0)
            ->where('pm_review_isDeleted', 0)
            ->whereNull('pm_review_sudah_dibaca')
            ->count();
    }

    public static function getDaftarReview()
    {
        // Mengambil daftar pengaduan masyarakat untuk review
        return self::where('isDeleted', 0)
            ->where('pm_review_isDeleted', 0)
            ->where('pm_status', '!=', 'Masuk')
            ->orderBy('created_at', 'desc')
            ->get();
    }
    ...
    public function validasiDanSetujuiReview($jawaban)
    {
        ...
        // Validasi status
        if ($this->pm_status !== 'Verifikasi') {
            throw new \Exception('Pengaduan harus dalam status Verifikasi untuk dapat direview');
        }
        ...
        $this->pm_status = 'Disetujui';
        $this->pm_jawaban = $jawaban;
        $this->pm_dijawab = $aliasReview;
        $this->pm_tanggal_dijawab = now();
        $this->save();
        ...
        $aktivitasSetuju = "{$namaReviewer} menyetujui review pengaduan masyarakat {$this->pm_jenis_laporan}";
        TransactionModel::createData(
            'APPROVED',
            $this->pengaduan_masyarakat_id,
            $aktivitasSetuju
    ...
    public function validasiDanTolakReview($alasanPenolakan)
    {
        ...
        $aktivitasTolak = "{$namaPenolak} menolak review pengaduan masyarakat {$this->pm_jenis_laporan} dengan alasan {$alasanPenolakan}";
        TransactionModel::createData(
            'REJECTED',
            $this->pengaduan_masyarakat_id,
            $aktivitasTolak
    ...
    public function validasiDanTandaiDibacaReview()
    {
        ...
        if (!in_array($this->pm_status, ['Disetujui', 'Ditolak'])) {
        ...
        // Tandai sebagai dibaca
        $this->pm_review_sudah_dibaca = $aliasDibaca;
    ...
    private function kirimEmailNotifikasiReview($status, $jawaban = null, $alasanPenolakan = null)
    {
        try {
            ...
            if (empty($email) || !$this->isValidEmail($email)) {
                Log::info("Email tidak valid atau kosong untuk review pengaduan masyarakat ID: {$this->pengaduan_masyarakat_id}");
                return;
            }

            // Kirim email
            try {
                Mail::to($email)->send(new ReviewPengaduanMasyarakatMail(
                    $this->pm_nama_tanpa_gelar,
                    $status,
                    $this->pm_jenis_laporan,
                    $this->pm_yang_dilaporkan,
    ...
    {
        try {
            Log::info("Starting WhatsApp review notification for pengaduan masyarakat ID: {$this->pengaduan_masyarakat_id}");

            // Ambil nomor HP pengadu
            $nomorHp = $this->pm_no_hp_pengguna;

            if (empty($nomorHp)) {
                Log::info("Nomor WhatsApp kosong untuk review pengaduan masyarakat ID: {$this->pengaduan_masyarakat_id}");
                return;
            }
            ...
            $pesanWhatsApp = $whatsappService->generatePesanReviewPengaduanMasyarakat(
                $this->pm_nama_tanpa_gelar,
                $status,
                $this->pm_jenis_laporan,
                $this->pm_yang_dilaporkan,
