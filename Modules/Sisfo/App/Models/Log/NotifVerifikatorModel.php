...
class NotifVerifikatorModel extends Model
{
    ...
    {
        return $this->belongsTo(PermohonanInformasiModel::class, 'notif_verikator_form_id', 'permohonan_informasi_id');
    }

    public function t_pernyataan_keberatan()
    {
        return $this->belongsTo(PernyataanKeberatanModel::class, 'notif_verikator_form_id', 'pernyataan_keberatan_id');
    }

    public function t_pengaduan_masyarakat()
    {
        return $this->belongsTo(PengaduanMasyarakatModel::class, 'notif_verikator_form_id', 'pengaduan_masyarakat_id');
    }

    public function t_wbs()
    {
        return $this->belongsTo(WBSModel::class, 'notif_verikator_form_id', 'wbs_id');
    }

    public function t_permohonan_perawatan()
    {
        return $this->belongsTo(PermohonanPerawatanModel::class, 'notif_verikator_form_id', 'permohonan_perawatan_id');
    }
    ...
    public static function tandaiDibaca($id)
    {
        ...
        $notifikasi->sudah_dibaca_notif_verif = now();
        $notifikasi->save();
    ...
    public static function hapusNotifikasi($id)
    {
        ...
        if (!$notifikasi->sudah_dibaca_notif_verif) {
            return [
                'success' => false,
    ...
    {
        $notifikasi = self::where('kategori_notif_verif', 'E-Form Permohonan Informasi')
            ->where('isDeleted', 0)
        ...
        foreach ($notifikasi as $item) {
            $item->sudah_dibaca_notif_verif = now();
            $item->save();
        }
    ...
    public static function hapusSemuaDibaca()
    {
        $notifikasi = self::where('kategori_notif_verif', 'E-Form Permohonan Informasi')
            ->where('isDeleted', 0)
            ->whereNotNull('sudah_dibaca_notif_verif')
            ->get();
